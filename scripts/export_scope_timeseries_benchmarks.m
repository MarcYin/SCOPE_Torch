function export_scope_timeseries_benchmarks(output_dir, step_indices)
%EXPORT_SCOPE_TIMESERIES_BENCHMARKS Export per-timestep MATLAB SCOPE fixtures.
%
% This exercises the upstream SCOPE time-series mode (simulation == 1) and
% writes one benchmark MAT file per timestep using the same field contract as
% export_scope_benchmark.m. The emitted fixtures can be compared with the
% existing Python parity harness step-by-step.

if nargin < 1 || isempty(output_dir)
    output_dir = fullfile(default_repo_root(), 'tests', 'data', 'timeseries_benchmark_fixtures');
end
...
vi = ones(length(V), 1);
for i = 1:length(V)
    if ~strcmp(V(i).Name, 'Tparam') && length(V(i).Val) > 1
        V(i).Val = V(i).Val(1);
    end
end

[soil, leafbio, canopy, meteo, angles, xyt] = select_input(V, vi, canopy, options, constants, struct(), struct(), struct());
[V, xyt, mly_ts, atmo_paths] = load_timeseries(V, F, xyt, path_input); %#ok<ASGLU>

if options.soil_heat_method < 2
    if isempty(meteo.Ta) || meteo.Ta < -273
        meteo.Ta = 20;
    end
...
if nargin < 2 || isempty(step_indices)
    step_indices = 1:telmax;
end
step_indices = step_indices(:)';
if any(step_indices < 1) || any(step_indices > telmax)
    error('step_indices must be between 1 and %d', telmax);
end
...
for step_index = step_indices
    vi = ones(nvars, 1);
    vi(vmax > 1) = step_index;
    ...
    if options.mSCOPE
        error('export_scope_timeseries_benchmarks does not currently support mSCOPE layered leaf inputs');
    end
    ...
    if ~isempty(atmo_paths) && step_index > 1
        atmfile = atmo_paths{step_index};
        atmo = load_atmo(atmfile, spectral.SCOPEspec);
    end
    ...
    if options.soil_heat_method < 2
        tsold_initial = soil.Tsold;
        if step_index > 1
            dt_seconds = (datenum(xyt.t(step_index)) - datenum(xyt.t(step_index - 1))) * 86400;
        else
            dt_seconds = 1 / 48 * 86400;
        end
    else
        tsold_initial = [];
        dt_seconds = nan;
    ...
    [iter, rad, thermal, soil, bcu, bch, fluxes, resistance, meteo] = ebal(constants, options, rad, gap, meteo, soil, canopy, leafbio, step_index, xyt, integr);
    ...
    benchmark = collect_scope_benchmark( ...
        spectral, optipar, leafbio, soil, canopy, angles, meteo, gap, rad, leafopt, thermal, ...
        bcu, bch, fluxes, resistance, iter, trace, fluor_diag, fluor_PoutFrc, fluor_EoutFrc, fluor_sigmaF, ...
        optical_diag, constants, options, F, parameter_files, integr, step_index ...
    );
    benchmark.simulation_mode = 'timeseries';
    benchmark.time_index = step_index;
    benchmark.time_timestamp = datestr(xyt.t(step_index), 'yyyy-mm-ddTHH:MM:SS.FFF');
    benchmark.time_year = xyt.year(step_index);
    benchmark.timeseries_length = telmax;
    benchmark.source_dataset_dir = F(5).FileName;
    benchmark.source_meteo_csv = F(6).FileName;
    benchmark.soil_heat_method = options.soil_heat_method;
    benchmark.energy_dt_seconds = dt_seconds;
    if ~isempty(tsold_initial)
        benchmark.soil_Tsold_initial = tsold_initial;
    end

    output_path = fullfile(output_dir, sprintf('scope_ts_step_%03d.mat', step_index));
    save(output_path, '-struct', 'benchmark');
    fprintf('Wrote time-series benchmark fixture to %s\n', output_path);
end
end


function benchmark = collect_scope_benchmark( ...
    spectral, optipar, leafbio, soil, canopy, angles, meteo, gap, rad, leafopt, thermal, ...
    bcu, bch, fluxes, resistance, iter, trace, fluor_diag, fluor_PoutFrc, fluor_EoutFrc, fluor_sigmaF, ...
    optical_diag, constants, options, F, parameter_files, integr, case_index)
...
benchmark.nlayers = canopy.nlayers;
...
benchmark.options_lite = options.lite;
benchmark.options_calc_fluor = options.calc_fluor;
benchmark.options_calc_planck = options.calc_planck;
benchmark.options_soilspectrum = options.soilspectrum;
end
...
vb = rad.vb(:, iwlfo); %#ok<NASGU>
vf = rad.vf(:, iwlfo); %#ok<NASGU>
